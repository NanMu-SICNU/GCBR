...
if __name__ == "__main__":
    ...
    for i in range(n_epochs):
        ...
        for f_img, f_label in zip(train_list, label_list):
            ...
            label = cv2.resize(label, (np.int(label_size), np.int(label_size)))
            ...
            label_flip = cv2.resize(label_flip, (np.int(label_size), np.int(label_size)))
    ...
    os.mkdir('ModelGB10')
    saver.save(sess, 'ModelGB10/model.ckpt', global_step=n_epochs)
